...
class AdminController extends Controller
{
    ...
    {
        // Use the new unified ReportService
        $reportService = app(\App\Services\ReportService::class);

        // Get all sections and HTEs for report generation
        $sections = $reportService->getSections($request->user());
        $htes = $reportService->getHTEs($request->user());
        $reportCategories = $reportService->getReportCategories('admin');
    ...
    public function exportSectionPDF(Request $request, $sectionId, $reportType): \Illuminate\Http\Response
    {
        ...
        $validReportTypes = ['student-list', 'student-assessment', 'placed-students', 'endorsed-students', 'assessment-summary', 'performance-analysis'];
    ...
    /**
     * Get section endorsed students
     */
    private function getSectionEndorsedStudents($sectionId): array
    {
    ...
    /* private function generateSectionCSVContent($sectionId, $reportType, $sectionName): string
    {
        ...
        switch ($reportType) {
            ...
            case 'endorsed-students':
                return $this->generateSectionEndorsedStudentsCSV($sectionId, $csvContent);
            case 'performance-analysis':
    ...
    private function generateSectionEndorsedStudentsCSV($sectionId, $csvContent): string
    {
        ...
        foreach ($endorsedStudents as $endorsement) {
            ...
            $csvContent .= $endorsement['section'] . ",";
            $csvContent .= '"' . $endorsement['company'] . '",';
            $csvContent .= '"' . $endorsement['position'] . '",';
            $csvContent .= '"' . $endorsement['department'] . '",';
    ...
    private function generateSectionExcelContent($sheet, $sectionId, $reportType, $sectionName)
    {
        ...
        switch ($reportType) {
            ...
            case 'placed-students':
                ...
                break;
            case 'endorsed-students':
                $this->generateEndorsedStudentsExcel($sheet, $sectionId, $row);
                break;
            case 'performance-analysis':
