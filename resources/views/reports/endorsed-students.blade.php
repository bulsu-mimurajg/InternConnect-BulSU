...
<body>
    <div class="container">
        <div class="content-wrapper">
            ...
            <div class="section">
                <h2 class="section-title">Endorsed Students</h2>
                @if($students->count() > 0)
                    <table>
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            @foreach($students as $student)
                                @php
                                    ...
                                    $internship = $endorsement ? $endorsement->internship : null;
                                    $hte = $internship ? $internship->hte : null;
                                @endphp
                                <tr>
                                    <td>
                                    ...
                                    <td>{{ $hte ? $hte->company_name : 'N/A' }}</td>
                                    <td>{{ $internship ? $internship->position_title : 'N/A' }}</td>
                                    <td>{{ $internship ? $internship->department : 'N/A' }}</td>
                                    <td>
                                        @if($endorsement && $endorsement->compatibility_score)
                                            <span style="color: {{ $endorsement->compatibility_score >= 80 ? '#27ae60' : ($endorsement->compatibility_score >= 60 ? '#f39c12' : '#e74c3c') }}; font-weight: bold;">
                                                {{ number_format($endorsement->compatibility_score, 1) }}%
                                            </span>
                                        @else
                                            N/A
                                        @endif
                                    </td>
                                    <td>{{ $endorsement ? $endorsement->endorsement_date->format('M d, Y') : 'N/A' }}</td>
                                    <td>
                                        <span style="color: {{ $endorsement && $endorsement->status === 'approved' ? '#27ae60' : '#e74c3c' }}; font-weight: bold;">
                                            {{ $endorsement ? ucfirst($endorsement->status) : 'N/A' }}
                                        </span>
                                    </td>
                                </tr>
                            @endforeach
                ...
                @else
                    <p style="text-align: center; color: #7f8c8d; font-style: italic; padding: 20px;">
                        No endorsed students found for the selected criteria.
                    </p>
                @endif
            </div>
